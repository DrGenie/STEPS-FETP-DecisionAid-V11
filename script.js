...
function exportScenariosToPdf() {
  ...
  showToast("PDF downloaded.", "success");
    ...
    function exportTop5OnlyExcel() {
      ...
      const items = computeTopScenarioItems(metric, feasibleOnly, selectedOnly, 5);
      if (!items.length) {
        showToast("No scenarios available for Top 5 export.", "warning");
        return;
      }
      ...
      rows.push([
        ...
        "Economic cost total (INR)",
        "Net benefit total (INR)",
        "Benefit–cost ratio",
      ...
      items.forEach((it, idx) => {
        const s = it.s;
        const c = s.config || {};
        const cap = it.cap || s.capacity || computeCapacity(c) || {};
        rows.push([
          idx + 1,
          c.name || ("Scenario " + (idx + 1)),
          c.tier || "",
          c.career || "",
          c.mentorship || "",
          c.delivery || "",
          c.cohorts || "",
          c.traineesPerCohort || "",
          s.endorseRate != null ? Number(s.endorseRate) : "",
          s.wtpPerTraineePerMonth != null ? Number(s.wtpPerTraineePerMonth) : "",
          s.costs && s.costs.totalEconomicCostAllCohorts != null ? Number(s.costs.totalEconomicCostAllCohorts) : "",
          s.netBenefitAllCohorts != null ? Number(s.netBenefitAllCohorts) : "",
          s.bcrAllCohorts != null ? Number(s.bcrAllCohorts) : "",
          cap.mentorsPerCohort != null ? Number(cap.mentorsPerCohort) : "",
          cap.totalMentorsRequired != null ? Number(cap.totalMentorsRequired) : "",
          cap.availableMentors != null ? Number(cap.availableMentors) : "",
          cap.mentorShortfall != null ? Number(cap.mentorShortfall) : "",
          cap.status || ""
        ]);
      });

      const sheet = XLSX.utils.aoa_to_sheet(rows);
      XLSX.utils.book_append_sheet(wb, sheet, "Top5");

      XLSX.writeFile(wb, "steps_top5_recommended.xlsx");
      showToast("Top 5 Excel file downloaded.", "success");
    }
      ...
      if (copilotOpenBtn) {
        copilotOpenBtn.addEventListener("click", () => {
          if (!output || !output.value) {
            const prompt = buildPrompt("copilot");
            output.value = prompt;
          }
